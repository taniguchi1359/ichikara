...
$posts = select('SELECT `posts`.`id`,`posts`.`title`,`posts`.`created_at`,`images`.`src`,`images`.`alt` FROM `posts` LEFT JOIN `images` ON `posts`.`icon`=`images`.`id`' . $where . ' LIMIT ' . $ARTICLE_PER_PAGE . ' OFFSET ' . $offset . ';');
...
# アーカイブHTML作成
foreach($ym as $post) {
	$d = date_parse($post['created_at']);
	if($py == $d['year'] && $pm == $d['month']) continue;
	$archives .= '<li><a href="?y=' . $d['year'] . '&m=' . $d['month'] . '">' . $d['year'] . '年' . $d['month'] . '月</a></li>';
	$py = $d['year'];
	$pm = $d['month'];
}
